...
export function VariantTab() {
  ...
  const removeVariant = async (variantKeyword: string) => {
    if (!confirm(`Remove "${variantKeyword}" as a variant?`)) return;
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="text-sm text-slate-500">
            {selectedParent ? (
              <span>
                Parent selected: <span className="font-medium">{selectedParent}</span>
              </span>
            ) : (
                  ...
                  <div key={group.parentKeyword} className="border border-slate-200 rounded-lg overflow-hidden">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <Badge variant="secondary">{group.variants.length} variant{group.variants.length !== 1 ? "s" : ""}</Badge>
                      </div>
                    </div>
